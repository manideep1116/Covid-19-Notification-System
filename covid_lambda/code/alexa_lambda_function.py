...
def on_intent_request(intent_request,local_file):
    ...
    # elif intent_name == "MOZoneIntent":
    #     return handle_mo_zone_intent(intent, local_file)
    # elif intent_name == "CountyZoneIntent":
    #     return handle_county_zone_intent(intent, local_file)
    # elif intent_name== "ZipcodeZoneIntent":
    #     return handle_zipcode_intent(intent, local_file)

    elif intent_name == "MOSurgeIntent":
        return handle_mo_surge_intent(intent, local_file)
    elif intent_name == "CountySurgeIntent":
        return handle_county_surge_intent(intent, local_file)
    elif intent_name== "ZipcodeSurgeIntent":
        return handle_zipcode_intent(intent, local_file)
    else:
...
def handle_covid_intent(intent, local_file):
    ...
    elif ('value' in intent['slots']['zone'] and 'value' in intent['slots']['zip']) or ('value' in intent['slots']['MO']):
...
def handle_saint_louis_intent(intent, local_file):
    ...
    data = pd.read_csv(local_file, encoding = 'utf-8').fillna(0)
    state = list(data['County'] .iloc[0:].values)
    if num == 1 or num ==0:
        options["SpeechText"] = "Here is the data from Manideep's covid prevention project, today's Covid cases in Saint Louis county are {}. ".format(state[-1])
    else:
        cases = ""
        for i in state[-num:]: cases+=str(i)+", "
...
def handle_zipcode_intent(intent, local_file):
    ...
    data = pd.read_csv(local_file, encoding = 'utf-8').fillna(0)
    state = list(data['Zip'] .iloc[0:].values)
    if num == 1 or num ==0:
        options["SpeechText"] = "Here is the data from Manideep's covid prevention project, today's Covid cases in zip code six three one four six {}. ".format(state[-1])
    else:
        cases = ""
        for i in state[-num:]: cases+=str(i)+", "
...
def handle_zipcode_surge_intent(intent, local_file):
    options={}
    return build_response(options)

def handle_county_surge_intent(intent, local_file):
    options={}
    return build_response(options)
